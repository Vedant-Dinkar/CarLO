...
const sendRequest = async (pam) => {
    const encodedParams = new URLSearchParams();
    encodedParams.set('content', pam);
    encodedParams.set('expectedCountries[0]', 'US');
    encodedParams.set('expectedCountries[1]', 'CA');
    encodedParams.set('expectedLanguages[0]', 'en');
    ...
    const options = {
      ...
      headers: {
        ...
        'X-RapidAPI-Key': '6e46358033mshcd3aaf3cbdfe265p1dd071jsn8bdff737d639',
    ...
    try {
      ...
      return response.data.result.score; // Return the score
    } catch (error) {
      console.error(error);
      return null; // Handle the error and return null (or any other default value)
    }
  };
...
app.get('/profile', isLoggedIn, async (req, res) => {
    try {
        ...
        if (car.length > 0) {
            ...
            var gaadi = await client.query('SELECT num, imagelink, model FROM cars WHERE num IN ($1)', [carNumbers]);
            gaadi = gaadi.rows;

            const profImage = req.user ? req.user.profileImage : null;
            res.render('user', { book: [], user: req.user.email, car: gaadi, n: 0, profImg: profImage });
        } else {
            // Handle case where there are no cars booked
            const profImage = req.user ? req.user.profileImage : null;
...
app.post('/rev', async(req, res) => {
    ...
    sendRequest(req.body.review).then((score) => {
